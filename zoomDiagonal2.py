...
#for count in range(0, framerate*duration):
    #start of iteration
    x = count/(2*framerate*duration)
    y = count/(2*framerate*duration)
    h =(980/3040 + ((maxH - startH)*count)/(framerate*duration))
    w =(1920/4056 + ((maxW - startW)*count)/(framerate*duration))
    #camera.zoom = (count/(2*framerate*duration) , count/(2*framerate*duration),
                 #  (1920/4056 + ((maxW - startW)*count)/(framerate*duration)),
                  # (980/3040 + ((maxH - startH)*count)/(framerate*duration)))
    camera.zoom = (x, y, w, h)
    #print(camera.zoom)
    sleep(1/zoomSpeed )
